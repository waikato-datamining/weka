...
public class ICSSearchAlgorithm extends CISearchAlgorithm {
   ...
   */
  void calcDependencyGraph(boolean[][] edges, SeparationSet[][] sepsets) throws Exception {
    /* calc undirected graph a-b iff D(a,S,b) for all S) */
    SeparationSet oSepSet;

    for (int iNode1 = 0; iNode1 < maxn(); iNode1++) {

      if ((iNode1 & 0xFF) == 0 && Thread.currentThread().isInterrupted()) throw new InterruptedException();
  ...
  SeparationSet existsSepSet(int iNode1, int iNode2, int nCardinality,
    boolean[][] edges) throws Exception {
  ...
  void calcVeeNodes(boolean[][] edges, boolean[][] arrows,
    SeparationSet[][] sepsets) throws Exception {
   ...
   */
  void calcArcDirections(boolean[][] edges, boolean[][] arrows) throws Exception {
