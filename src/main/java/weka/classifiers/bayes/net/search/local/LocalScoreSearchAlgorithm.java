...
public class LocalScoreSearchAlgorithm extends SearchAlgorithm {
   ...
   * @return log score.
   */
  public double logScore(int nType) throws Exception {
    ...
    for (int iAttribute = 0; iAttribute < instances.numAttributes(); iAttribute++) {
      ...
      if ((iAttribute & 0xFF) == 0 && Thread.currentThread().isInterrupted()) throw new InterruptedException();
   ...
   * @return log score
   */
  public double calcNodeScore(int nNode) throws Exception {
   ...
   * @return log score
   */
  private double calcNodeScoreADTree(int nNode) throws Exception {
    Instances instances = m_BayesNet.m_Instances;
  ...
  private double calcNodeScorePlain(int nNode) throws Exception {
    Instances instances = m_BayesNet.m_Instances;
    ParentSet oParentSet = m_BayesNet.getParentSet(nNode);
    ...
    while (enumInsts.hasMoreElements()) {
      ...
      for (int iParent = 0; iParent < oParentSet.getNrOfParents(); iParent++) {
        ...
        if ((iParent & 0xFF) == 0 && Thread.currentThread().isInterrupted()) throw new InterruptedException();
  ...
  protected double calcScoreOfCounts(int[] nCounts, int nCardinality,
    int numValues, Instances instances) throws Exception {
