...
public class RepeatedHillClimber extends HillClimber {
  ...
  void generateRandomNet(BayesNet bayesNet, Instances instances) throws Exception {
    ...
    for (int iAttempt = 0; iAttempt < nNrOfAttempts; iAttempt++) {
      ...
      if ((iAttempt & 0xFF) == 0 && Thread.currentThread().isInterrupted()) throw new InterruptedException();
