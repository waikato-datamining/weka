...
public class SearchAlgorithm implements OptionHandler, Serializable,
   ...
   * iAttributeHead does not already exist and does not introduce a cycle
   * 
   * @param bayesNet
   * @param instances
  ...
  protected boolean addArcMakesSense(BayesNet bayesNet, Instances instances,
    int iAttributeHead, int iAttributeTail) throws Exception {
    ...
    for (int iNode = 0; iNode < nNodes; iNode++) {
      ...
      if ((iNode & 0xFF) == 0 && Thread.currentThread().isInterrupted()) throw new InterruptedException();
  ...
  protected boolean reverseArcMakesSense(BayesNet bayesNet,
    Instances instances, int iAttributeHead, int iAttributeTail) throws Exception {
  ...
  protected boolean isArc(BayesNet bayesNet, int iAttributeHead,
    int iAttributeTail) throws Exception {
