...
public class PropertyDialog
  ...
  protected void initialize(PropertyEditor pe, int x, int y) {
    ...
    // Some PropertyEditors (e.g., JDK StringEditor) may return null here.
    if (m_EditorComponent != null) {
      getContentPane().add(m_EditorComponent, BorderLayout.CENTER);
    } else if (pe.getAsText() != null) {
      getContentPane().add(new PropertyText(pe), BorderLayout.CENTER);
    } else {
      getContentPane().add(
              new JLabel("No custom editor component available for: " + pe.getClass().getName(),
