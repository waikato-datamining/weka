...
public class RunPanel
  ...
  protected volatile Thread m_RunThread = null;
  ...
  class ExperimentRunner
	...
	if (m_ExpCopy instanceof RemoteExperiment) {
	  ...
	  ((RemoteExperiment)m_ExpCopy).
		...
		public void remoteExperimentStatus(final RemoteExperimentEvent e) {
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
	...
	if (!(m_ExpCopy instanceof RemoteExperiment)) {
	  ...
	  while (!Thread.currentThread().isInterrupted() && m_ExpCopy.hasMoreIterations()) {
	    try {
	...
	if (m_ResultsPanel != null) {
        final Experiment expCopyFinal = m_ExpCopy;
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                m_ResultsPanel.setExperiment(expCopyFinal);
            }
        });
	}
	if (!(m_ExpCopy instanceof RemoteExperiment)) {
  ...
  public void actionPerformed(ActionEvent e) {
	...
	logMessage("Waiting for remote tasks to "
      ...
      Thread t = m_RunThread;
      if (t != null) {
          if (t instanceof ExperimentRunner) {
              ((ExperimentRunner) t).abortExperiment();
          }
          t.interrupt();
      }
    }
